...
namespace ReviewHelp.Toolbox.Tests.Infrastructure
{
	internal static class TestHelper
	{
		...
		static TestHelper()
		{
			...
			var refs = referencedAssemblies.Where(x => !ignore.Contains(x.Name)).Select(x => (MetadataReference)MetadataReference.CreateFromFile(Assembly.Load(x.FullName).Location)).Where(x => x != null);
