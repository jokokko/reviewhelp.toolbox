...
Target "Build" (fun _ ->
  ...
  CreateCSharpAssemblyInfo @".\src\Version.cs"
    [ Attribute.Title "ReviewHelp.Toolbox"
      Attribute.Description "Supplemental tools for ReviewHelp.Analyzers"
      Attribute.Product "ReviewHelp.Toolbox"
  ...
  build setParams @"./src/ReviewHelp.Toolbox.sln"
      |> DoNothing
)
...
Target "Package" (fun _ ->    
    let zipFiles = CreateZip "." (buildArtifactPath @@ ("ReviewHelp.Toolbox." + Version + ".zip")) "" DefaultZipLevel true
    !! (sprintf "./src/ReviewHelp.Toolbox/bin/%s/*.dll" buildMode)
    ++ (sprintf "./src/ReviewHelp.Toolbox/bin/%s/*.exe" buildMode)
    ++ (sprintf "./src/ReviewHelp.Toolbox/bin/%s/*.exe.config" buildMode)    
    |> Seq.toArray
    |> zipFiles
)
